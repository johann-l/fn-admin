...
export async function createCheckoutSession(formData: FormData) {
  ...
  if (!expenseId || !expenseDescription || !expenseAmount) {
    ...
    throw new Error(
      "Missing required expense data (ID, description, or amount)."
    );
  }
  ...
  // In Next.js, dynamic APIs like headers() must be awaited
  const hdrs = await headers();
  const host = hdrs.get("host") || "localhost:9002";
...
export async function createTransactionCheckoutSession(formData: FormData) {
  ...
  const transactionDescription = formData.get(
  ...
  // Await headers() to comply with Next.js dynamic API requirements
  const hdrs = await headers();
  const host = hdrs.get("host") || "localhost:9002";
  const protocol = process.env.NODE_ENV === "development" ? "http" : "https";
  const appUrl = `${protocol}://${host}`;
