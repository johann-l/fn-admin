...
export default function SummaryCards() {
  ...
  const [selectedAlert, setSelectedAlert] = React.useState<Alert | null>(
    activeAlerts.length > 0 ? activeAlerts[0] : null
  );
          ...
          {vehicles && vehicles.length > 0 ? (
            <>
              ...
              <p className="text-xs text-muted-foreground">
                {
                  vehicles.filter((v) => v.status?.toLowerCase() === "on time")
                    .length
                }{" "}
                On Time,{" "}
                {
                  vehicles.filter((v) => v.status?.toLowerCase() === "delayed")
                    .length
                }{" "}
                Delayed,{" "}
                {
                  vehicles.filter((v) => v.status?.toLowerCase() === "early")
                    .length
                }{" "}
                Early
              </p>
