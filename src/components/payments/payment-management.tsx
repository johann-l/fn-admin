      ...
      if (transactionId) {
        ; (async () => {
          const { error } = await supabase
            .from("transactions")
            .update({ status: "Paid", reference_id: `stripe_${Date.now()}` })
            .eq("id", transactionId)
  ...
  const onSubmit = async (values: PaymentFormValues) => {
    try {
      ...
      toast({
        ...
        description: err?.message || err?.hint || "Failed to create payment. Check console for details.",
        variant: "destructive",
      })
    }
  }
              ...
              <div onClick={(e) => e.stopPropagation()}>
                <TooltipProvider>
                  <form action={createTransactionCheckoutSession}>
                    ...
                    <input type="hidden" name="transactionDescription" value={payment.description} />
                    <input type="hidden" name="transactionAmount" value={String(payment.amount)} />
                    <input type="hidden" name="transactionType" value={payment.type} />
                    <PayButton />
                  </form>
                </TooltipProvider>
              </div>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Input {...field} placeholder="e.g., Bus Pass Fee - Student" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
